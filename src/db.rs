...
pub async fn all_token(connection_pool: &PgPool, offset: i64, limit: i64) -> Result<Vec<Token>> {
    let sql = "SELECT token_id AS id, 
        encode(contract_addr, 'hex') AS contract_addr, 
        last_checked_block, symbol, decimals FROM token ORDER BY symbol OFFSET $1 LIMIT $2";
...
pub async fn holder_id_by_holder_addr(
...
pub async fn token_id_by_contract_addr(
    connection_pool: &PgPool,
    contract_addr: String,
) -> Result<i32> {
    let sql = "SELECT token_id FROM token WHERE contract_addr = decode($1, 'hex')";
